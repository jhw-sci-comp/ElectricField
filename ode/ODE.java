...
public class ODE {
	...
	public static Vector2D solveODEStep(Vector2D r, float h, Function<Vector2D, Vector2D> func) {
		...
		do {	
			...
			normalized_func = VectorSpace2D.calculate2Norm(func.apply(r_new.add(k1.scale(h * 2.0f / 9.0f))));
			k2 = func.apply(r_new.add(k1.scale(h * 2.0f / 9.0f))).scale(1.0f / normalized_func);
			
			normalized_func = VectorSpace2D.calculate2Norm(func.apply(r_new.add(k1.scale(h / 12.0f).add(k2.scale(h / 4.0f)))));
			k3 = func.apply(r_new.add(k1.scale(h / 12.0f).add(k2.scale(h / 4.0f)))).scale(1.0f / normalized_func);
			
			normalized_func = VectorSpace2D.calculate2Norm(func.apply(r_new.add(k1.scale(h * 69.0f / 128.0f).add(k2.scale(-h * 243.0f / 128.0f).add(k3.scale(h * 135.0f / 64.0f))))));
			...
			normalized_func = VectorSpace2D.calculate2Norm(func.apply(r_new.add(k1.scale(-h * 17.0f / 12.0f).add(k2.scale(h * 27.0f / 4.0f).add(k3.scale(-h * 27.0f / 5.0f).add(k4.scale(h * 16.0f / 15.0f)))))));
			...
			normalized_func = VectorSpace2D.calculate2Norm(func.apply(r_new.add(k1.scale(h * 65.0f / 432.0f).add(k2.scale(-h * 5.0f / 16.0f).add(k3.scale(h * 13.0f / 16.0f).add(k4.scale(h * 4.0f / 27.0f).add(k5.scale(h * 5.0f / 144.0f))))))));
